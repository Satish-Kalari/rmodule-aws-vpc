resource "aws_vpc" "main" {
  ...
  # merging common tags and resource tags 
  tags = merge( 
    var.common_tags,
    var.vpc_tags,
    {
...
# aws available zones (az) are fetching using data source and take first two available
...
resource "aws_nat_gateway" "main" {
  ...
  # NAT gate way should be in any public subnet for internet access
...
# since there are more than one subnet in public, private and database subnets need to loop using length function in count

# element retrieves a single element from a list.
# > element(["a", "b", "c"], 1)
# b

# to get all sunents using *

# count.index will lopp both subnet_cidrs